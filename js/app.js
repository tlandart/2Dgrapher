const c = document.getElementsByClassName("graph-inner")[0];
fitCanvas(c);
...
const pl = document.getElementsByClassName("menu-label")[0];
const ip = document.getElementsByClassName("menu-input")[0];
...
class RealText {
    ...
    equals(o) {
        if (o === undefined)
            return false;
        else if (!(o instanceof RealText))
            return false;
        else
            return (this.text === o.text && this.color === o.color && this.point.equals(o.point));
    }
    // return a deep copy of the object
    copy() {
...
class Connection {
    ...
    toScreenPoint(p) {
        ...
        */
        return new ScreenPoint(cWidth / 2 + STANDARD_GRID_SIZE * this.scale * p.x, cHeight / 2 - STANDARD_GRID_SIZE * this.scale * p.y);
    }
    toRealPoint(p) {
        // reverse the operations that we did in toScreenPoint()
        p = new ScreenPoint((p.x - cWidth / 2) / (STANDARD_GRID_SIZE * this.scale), -(p.y - cHeight / 2) / (STANDARD_GRID_SIZE * this.scale));
        return new RealPoint(p.x + this.cameraPos.x, p.y + this.cameraPos.y);
    }
    // helper
    isOnScreen(p) {
        // determines if a RealPoint is on screen or not
        let pScreen = this.toScreenPoint(p);
        if (pScreen.x > cWidth || pScreen.x < 0 || pScreen.y > cHeight || pScreen.y < 0)
            return false;
        return true;
    }
    // draw a line with two RealPoints
    drawLine(start, end, colorHex, thickness) {
    ...
    drawAxis(vertical, point, colorHex, thickness) {
        if (colorHex === undefined)
            colorHex = "#ffffff";
        if (thickness === undefined)
            thickness = 2;
    ...
    drawCircle(center, radius, colorHex, full, thickness) {
        if (colorHex === undefined)
            colorHex = "#ffffff";
        if (thickness === undefined)
            thickness = 2;
    ...
    drawText(topLeft, text, colorHex, strokeHex, fontsize) {
    ...
    drawFunction(theFunction, colorHex, thickness) {
        const LEFT_X = this.toRealPoint(new ScreenPoint(-10, cHeight / 2)).x;
        const RIGHT_X = this.toRealPoint(new ScreenPoint(cWidth + 10, cHeight / 2)).x;
        // the real length of each line segment that will be rendered
        const SEGMENT_LENGTH = (RIGHT_X - LEFT_X) / FUNCTION_RESOLUTION;
        for (let xNew = LEFT_X; xNew <= RIGHT_X; xNew += SEGMENT_LENGTH) {
            if (theFunction.f(xNew) !== undefined && theFunction.f(xNew + SEGMENT_LENGTH) !== undefined) {
                let start = new RealPoint(xNew, theFunction.f(xNew));
                let end = new RealPoint(xNew + SEGMENT_LENGTH, theFunction.f(xNew + SEGMENT_LENGTH));
    ...
    drawGrid() {
        ...
        let axisPoint = this.toRealPoint(new ScreenPoint(0, cHeight / 2));
        ...
        this.lines.forEach(line => {
            ...
            let a = connection.toRealPoint(new ScreenPoint(0, 0));
            let b = connection.toRealPoint(new ScreenPoint(ARROW_LENGTH, 0));
            const ARROW_LENGTH_REAL = Math.abs(a.x - b.x);
            ...
            if (pointDistance(line.start, line.end) >= ARROW_LENGTH_REAL) {
                ...
                function rotateAroundPoint(theta, origin, p) {
                    return new RealPoint(Math.cos(theta) * (p.x - origin.x) - Math.sin(theta) * (p.y - origin.y) + origin.x, Math.sin(theta) * (p.x - origin.x) - Math.cos(theta) * (p.y - origin.y) + origin.y);
                }
                ...
                let theta = Math.atan2(lineCopy.end.y, lineCopy.end.x);
                // 3. create two points for the ends of the arrow and rotate them around the end point to the right angle
                let arrowEnd1 = new RealPoint(line.end.x + ARROW_LENGTH_REAL, line.end.y);
                let arrowEnd2 = new RealPoint(line.end.x + ARROW_LENGTH_REAL, line.end.y);
    ...
    updateMenu() {
        let x = this.scale > 10 ? this.cameraPos.x : +this.cameraPos.x.toFixed(2);
        let y = this.scale > 10 ? this.cameraPos.y : +this.cameraPos.y.toFixed(2);
...
function isScreenPointNear(p1, p2) {
...
onmousemove = (e) => {
    if (document.elementFromPoint(mouseXCurrent, mouseYCurrent) === c) {
        ...
        const OLD_POINT = connection.toRealPoint(new ScreenPoint(mouseXCurrent, mouseYCurrent));
        const NEW_POINT = connection.toRealPoint(new ScreenPoint(e.clientX, e.clientY));
        // convert CLOSE_DISTANCE from amount of pixels to a distance on the real graph
        let a = connection.toRealPoint(new ScreenPoint(0, 0));
        let b = connection.toRealPoint(new ScreenPoint(CLOSE_DISTANCE, 0));
        const CLOSE_DISTANCE_REAL = Math.abs(a.x - b.x);
        ...
        for (let i = 0; i < connection.lines.length; i++) {
            ...
            let closestPoint = new RealPoint(lineX, lineY);
            let closestScreenPoint = connection.toScreenPoint(closestPoint);
            // find distance from mouse to that point on the line (in pixels, not on the real grid)
            let dist = pointDistance(closestScreenPoint, new ScreenPoint(e.clientX, e.clientY));
            // if its not in bounds, then make it so it won't draw the label later on
            if (line.end.x >= line.start.x) { // x bounds
                if (closestPoint.x > line.end.x + CLOSE_DISTANCE_REAL / 2 || closestPoint.x < line.start.x - CLOSE_DISTANCE_REAL / 2)
                    dist = CLOSE_DISTANCE + 1;
            }
            else {
                if (closestPoint.x < line.end.x - CLOSE_DISTANCE_REAL / 2 || closestPoint.x > line.start.x + CLOSE_DISTANCE_REAL / 2)
                    dist = CLOSE_DISTANCE + 1;
            ...
            if (line.end.y >= line.start.y) { // y bounds
                if (closestPoint.y > line.end.y + CLOSE_DISTANCE_REAL / 2 || closestPoint.y < line.start.y - CLOSE_DISTANCE_REAL / 2)
                    dist = CLOSE_DISTANCE + 1;
            }
            else {
                if (closestPoint.y < line.end.y - CLOSE_DISTANCE_REAL / 2 || closestPoint.y > line.start.y + CLOSE_DISTANCE_REAL / 2)
        ...
        for (let i = 0; i < connection.functions.length; i++) {
            ...
            let theFunction = connection.functions[i];
            // the point on the function at the same x value as the mouse
            let functionXPoint = new RealPoint(NEW_POINT.x, theFunction.f(NEW_POINT.x));
            // create the first order taylor series (tangent line) at the mouse x point
            // f(x) = f(a)+f'(a)(x-a)
            let taySeries = new RealFunction(theFunction.f(functionXPoint.x).toString() + " + " + theFunction.derivative().f(functionXPoint.x).toString() + "(x-" + functionXPoint.x.toString() + ")");
            // make sure the line segment we will be using is the same length as CLOSE_DISTANCE_REAL (we'll do that iteratively)
            let deltaX = 0;
            let taySeriesStart = new RealPoint(NEW_POINT.x - deltaX, taySeries.f(NEW_POINT.x - deltaX));
            let taySeriesEnd = new RealPoint(NEW_POINT.x + deltaX, taySeries.f(NEW_POINT.x + deltaX));
            let pSize = CLOSE_DISTANCE_REAL / CLOSE_DISTANCE; // the real distance of one pixel
            ...
            let taySeriesPoint = new RealPoint(lineX, lineY);
            let taySeriesScreenPoint = connection.toScreenPoint(taySeriesPoint);
            // find distance from mouse to the point on the taylor series (in pixels, not on the real grid)
            let dist = pointDistance(taySeriesScreenPoint, new ScreenPoint(e.clientX, e.clientY));
            // if its not in bounds, then make it so it won't draw the label
            if (taySeriesPoint.x > taySeriesEnd.x && taySeriesPoint.x < taySeriesStart.x)
                dist = CLOSE_DISTANCE + 1; // x bounds
            if (taySeriesEnd.y >= taySeriesStart.y) { // y bounds
                if (taySeriesPoint.y > taySeriesEnd.y || taySeriesPoint.y < taySeriesStart.y)
                    dist = CLOSE_DISTANCE + 1;
            }
            else {
                if (taySeriesPoint.y < taySeriesEnd.y || taySeriesPoint.y > taySeriesStart.y)
        ...
        if (!isLabel && connection.label.text.length > 0)
            connection.label = new RealText();
...
function loadFunction() {
    var functionInput = document.getElementById("function-input-box");
...
function loadPoint() {
    const REX = /\(.*,.*\)/gm;
    const pointInput = document.getElementById("point-input-box");
    const INPUT = pointInput.value;
    pointInput.value = "";
    if (REX.test(INPUT)) {
        let commaPosition = INPUT.indexOf(",");
        let inputSplit = [];
...
function loadLine() {
    ...
    const REX = /\(.*,.*\),\(.*,.*\)/gm;
    const lineInput = document.getElementById("line-input-box");
...
function teleportToInput(event) {
    if (event.key === "Enter") {
        const REX = /\(.*,.*\)/gm;
        let pointInput = document.getElementsByClassName("menu-input")[0];
        const INPUT = pointInput.value;
        pointInput.value = "";
        if (REX.test(INPUT)) {
            let commaPosition = INPUT.indexOf(",");
            ...
            let teleportPoint = new RealPoint(inputFinal[0], inputFinal[1]);
...
function initTheme() {
    light = false;
    let button = document.getElementById("theme-image");
    ...
    if (!light) { // if the theme is dark we need to switch the menu colors
        ...
        let temp = getComputedStyle(root).getPropertyValue("--primary");
        let temp2 = getComputedStyle(root).getPropertyValue("--primarydark");
...
function changeTheme() {
    ...
    let starttime;
    function incrementTheme(timestamp, duration) {
